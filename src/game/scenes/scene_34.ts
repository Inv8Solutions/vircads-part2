...
export class Scene34 extends Scene {
    ...
    create() {
        ...
        this.add.image(width / 2, height / 2, 'scene_34').setDisplaySize(width, height);

        // UI scaffold placeholder
        const uiContainer = this.add.container(0, 0).setDepth(1000);
        const label = this.add.text(12, 12, 'scene_34 UI scaffold', { font: '16px Arial', color: '#ffffff' }).setDepth(1000);
